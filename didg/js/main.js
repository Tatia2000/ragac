jQuery(document).on('ready', function ($) {
    "use strict";
    $('body').scrollspy({target: '.bs-example-js-navbar-scrollspy', offset: 50});
    $("#mainmenu-area").sticky({topSpacing: 0});
    var mySlider = $('.pogoSlider').pogoSlider({pauseOnHover: false}).data('plugin_pogoSlider');
    ...
    var input = $('#time').clockpicker({placement: 'bottom', align: 'left', autoclose: true, 'default': 'now'});
    ...
    $('a.scrolltotop, .slider-area h3 a, .navbar-header a, ul#nav a').on('click', function (event) {
        ...
        $('html, body').animate({scrollTop: target}, 1500, "easeInOutExpo");
        event.preventDefault();
    });
    ...
    $('.team-slider').owlCarousel({
        ...
        margin: 15,
        responsiveClass: true,
        responsive: {0: {items: 1}, 600: {items: 3}, 1000: {items: 4}, 1200: {items: 5}}
    });
    $('.post-slider').owlCarousel({
        ...
        margin: 15,
        responsiveClass: true,
        responsive: {0: {items: 1}, 600: {items: 1}, 1000: {items: 2}, 1200: {items: 3}}
    });
